      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
        <div className="bg-white px-8 py-10 rounded-xl border border-gray-200">
          ...
          <div className="flex flex-col sm:flex-row sm:items-baseline gap-4 mb-10 pb-6 border-b border-gray-200">
            ...
            <span className="text-gray-500 font-medium text-lg">#{order._id.toUpperCase()}</span>
          </div>

          <div className="flex flex-col gap-12 lg:gap-16">
            {order.items.map((item, idx) => (
              <div key={idx} className="flex flex-col lg:flex-row gap-8 lg:gap-12 justify-between pb-10 border-b border-gray-100 last:border-0 last:pb-0">
                {/* Left Side: Tracking Info */}
                <div className="flex-1">
                  <TrackingInfoBox status={order.status} trackingId={order.trackingId} />
                </div>
